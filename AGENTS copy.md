...
| `--affinity_mw_correction` | ✓ | 分子量校正，必须开启 |
| `--diffusion_samples 1` | 1 | 结构预测采样数（每个 case 保存 1 个结构） |
| `--diffusion_samples_affinity` | 5（CLI 默认） | affinity 专用采样数，独立于结构预测 |
| `--recycling_steps 3` | 3 | 结构预测 recycling 步数 |
