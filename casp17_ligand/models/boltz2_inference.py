...
def main(cfg: DictConfig) -> None:
    ...
    for yaml_file in yaml_files:
        ...
        # Boltz creates: {target_output_dir}/boltz_results_{stem}/predictions/B/*.cif
        # Check actual CIF files, not just directory existence (empty dirs = failed runs)
        cif_dir = target_output_dir / f"boltz_results_{yaml_file.stem}" / "predictions" / "B"
        if not cfg.override and cif_dir.exists() and list(cif_dir.glob("*.cif")):
