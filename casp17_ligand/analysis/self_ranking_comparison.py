...
def _entry_chain_count(inner) -> int:
    """How many physical chains one input entry declares.

    Homodimers/multimers are expressed differently per method:
      AF3      `"id": ["A", "B"]`   (list of chain IDs)
      Protenix `"count": 2`
      Seedfold `"copies": 2`
    A scalar `id` or a missing count means a single chain.
    """
...
def _parse_input_entities(
    ...
    Each method has its own input schema:
      ...
      - Protenix: sequences → proteinChain / ligand / ion
      - Seedfold: entities  → Protein / Ligand-Smiles
      - Boltz2:   YAML sequences → protein / smiles / sdf / ccd
                  (NOT parseable here — this function is JSON-only; callers must
                   use the `n_protein_chains` override for Boltz2.)

    The returned indices are **chain** indices matching the rows of the
    `pair_chains_iptm` matrix, so an entry declaring several copies expands to
    that many consecutive indices.  A homodimer can be written either way:
        AF3 compact : {"protein": {"id": ["A", "B"], ...}}      → 2 chains
        AF3 explicit: {"protein": {"id": "A"}}, {"protein": ...} → 2 chains
        Protenix    : {"proteinChain": {"count": 2, ...}}        → 2 chains
        Seedfold    : {"entity": "Protein", "copies": 2, ...}    → 2 chains
    Counting entries instead of copies silently shifts every downstream chain
    index (a 4-chain dimer would be read as 2 chains), which makes
    `extract_pair_chains_iptm` return the protein–protein interface value.

    Protenix routes metal ions through a separate ``ion`` key rather than
    ``ligand``.  That entity still occupies a chain in the confidence matrix, so
    it has to be counted here: skipping it makes the parsed chain total disagree
    with the matrix, and the caller then discards the whole parse and falls back
    ...
    if method == "seedfold" and "entities" in inp:
        ...
        for ent in inp["entities"]:
            ...
            n_copies = _entry_chain_count(ent)
    ...
    for seq_entry in sequences:
        if isinstance(seq_entry, dict):
            for key in seq_entry:
                kl = key.lower()
                inner = seq_entry[key]
                n_copies = _entry_chain_count(inner)
                smi = ""
                ...
                for _ in range(n_copies):
                    if "protein" in kl:
                        protein_indices.append(idx)
                    elif "ligand" in kl or kl in ("smiles", "sdf", "ccd", "ion"):
                        ligand_indices.append(idx)
                        ligand_smiles[idx] = smi
                    idx += 1
                # Only the first key per entry describes the entity
                break
...
def extract_pair_chains_iptm(
    ...
    Steps:
      ...
      2. Drop the trailing `n_cofactor_chains` ligand chains (see below).
      3. Among the rest, find the largest (by heavy-atom count). If tied, keep all.
      4. Return the max cross-chain value among (protein, target_ligand) pairs.
      5. Fallback: assume the first `n_protein_chains` chains are protein.

    `n_protein_chains` (default 0 → treated as 1) drives the fallback used when
    the input file is missing or disagrees with the confidence matrix.  It MUST
    be set for multimers: Boltz-2 inputs are YAML and cannot be parsed here at
    all, so a homodimer would otherwise fall back to "chain 0 is the protein,
    chains 1..n are ligands" and report the protein–protein interface iptm
    (~0.9 for a well-folded dimer) instead of the protein–ligand value.

    `n_cofactor_chains` (default 0 → nothing dropped) names how many trailing
    ligand chains hold a catalytic cofactor supplied with the target rather than
    the molecule being predicted — the CASP17 L-series ships one ZN or SFG per
    structure.  Without it, a cofactor chain competes in the max at step 4 and
    wins, because protein–cofactor iptm runs higher than protein–fragment: the
    score then ranks models on how confidently they placed a cofactor whose
    position was effectively given.  Heavy-atom count cannot substitute for this
    flag.  It excludes the cofactor only by accident, when the cofactor is
    declared as a CCD code that RDKit fails to parse to 0 heavy atoms; where the
    ligands carry no parseable SMILES at all (Boltz-2, whose YAML input is never
    read here) every chain ties at 0 and the cofactor stays in the running.
    Cofactors are assumed to be declared last, which is how the three L-series
    input generators emit them and how all three engines order the output chains.
    """
    ...
    # The parsed chain count must agree with the confidence matrix, otherwise the
    # indices refer to a different chain numbering than `pci` and every lookup
    # below would silently read the wrong cell.
    parsed_total = len(protein_indices) + len(ligand_indices)
    if parsed_total != n_chains:
        if parsed_total:
            warnings.warn(
                f"{method}: input file declares {parsed_total} chains but the "
                f"confidence matrix has {n_chains}; falling back to "
                f"n_protein_chains={n_protein_chains or 1}"
            )
        protein_indices, ligand_indices, ligand_smiles = [], [], {}

    # Fallback if no input file (or it disagreed with the matrix)
    if not protein_indices or not ligand_indices:
        npc = n_protein_chains if n_protein_chains > 0 else 1
        if n_chains > npc:
            protein_indices = list(range(npc))
            ligand_indices = list(range(npc, n_chains))
        else:
            return None
    ...
    # --- Drop supplied cofactor chains before choosing the target ligand ---
    if n_cofactor_chains > 0:
        if len(ligand_indices) > n_cofactor_chains:
            ligand_indices = sorted(ligand_indices)[:-n_cofactor_chains]
        else:
            warnings.warn(
                f"{method}: n_cofactor_chains={n_cofactor_chains} would consume "
                f"all {len(ligand_indices)} ligand chain(s); keeping them all"
            )
...
def _get_ensemble_cif_list(method: str, target: str, method_output_dir) -> List[str]:
    ...
    if method == "boltz2":
        ...
        # Match either case prefix (R2314 used lowercase `r2314_*`, CASP17
        # R2317/R2318 use capital `R2317_*`). Order via dedup so idx mapping
        # stays stable.
        if os.path.isdir(method_output_dir):
            ...
            seen: set = set()
            for pat in patterns:
                for cif in sorted(glob.glob(os.path.join(
