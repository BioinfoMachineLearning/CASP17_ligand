...
def _resolve_af3_base(method_output_dir: str, target: str) -> str:
    """AF3 has two possible layouts: nested {TARGET}/{target_lower}/ and flat {target_lower}/."""
...
def _iter_confidence_jsons(method: str, target: str, method_output_dir):
    ...
    if method == "af3":
        ...
        any_nested = False
        # Glob `{target_lower}*_seed-*` matches both canonical `r2314_seed-*` and
        # round-suffixed `r2314_r1_seed-*` (CASP17 convention with name=R2314_r1).
        ...
        # Flat dataset layout: all targets share method_output_dir; fall through
        # to glob at that level (l1000/l2000/l4000 r2 style).
        if not any_nested:
            for jf in sorted(glob.glob(os.path.join(
