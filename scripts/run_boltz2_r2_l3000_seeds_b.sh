#!/bin/bash
# L3000 R2 - Seeds group B (617,789,888,1024,2025) on GPU 1
set -uo pipefail
...
SEEDS=(617 789 888 1024 2025)

echo "=== L3000 R2 Seeds B: ${SEEDS[*]} ==="
    ...
    for yaml_file in "${INPUT_DIR}"/*_input.yaml; do
        ...
        if [ -d "${cif_dir}" ]; then
            ...
            if [ "${cif_count}" -ge "${DIFFUSION_SAMPLES}" ]; then
                echo "  [${target}] seed=${seed}: skip (${cif_count} CIFs)"
                continue
            fi
        fi
        ...
        ${BOLTZ_EXEC} predict "${yaml_file}" \
            ...
            --seed "${seed}" \
            --devices 1 \
            --accelerator gpu || echo "  [${target}] seed=${seed}: FAILED"
    done
done
echo "=== Seeds B complete ==="
