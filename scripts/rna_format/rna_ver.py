...
def format_check(fpdb):
    ...
    try:
        ...
        sys.stderr.write('# ERROR! target structure file format error <%s>\n'%fref)
        print( Exception)
        return False
    atm_list=[]
    ...
    for i,mdl in enumerate(struct.child_list):
        ...
        for j,fg in enumerate(flag):
            if not fg:
                n_err+=1
               # print(f"EE {i}")
                err_log+='# ERROR! Chain %s Residue %s %s atom %s present in the template not found in the model %d counting from the top of the prediction file \n'%(atm_list[j].split()[0],atm_list[j].split()[2],atm_list[j].split()[4],atm_list[j].split()[8][:-1],i+1)
...
if __name__ == '__main__':
    ...
    if format_check(sys.argv[1]):
        print( 'RNA model : Format check passed!')
    else:
        print( '# ERROR! Please fix errors in RNA structure and re-submit! Make sure that chain identifiers in your model are matching those in the fasta sequence and in the PDB template.')
