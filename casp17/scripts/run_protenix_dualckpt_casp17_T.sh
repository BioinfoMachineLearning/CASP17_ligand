#!/bin/bash
# CASP17 T-series Protenix dual-ckpt protocol (locked-in 2026-06-13, ported
...
#   - INPUT_JSON path / output dirs use casp17_T
#   - Config names: protenix_inference_casp17_T_r{1,2}
...
#
# Usage:
#   bash casp17/scripts/run_protenix_dualckpt_casp17_T.sh <TARGET> <GPU_A> <GPU_B>
# Example:
#   bash casp17/scripts/run_protenix_dualckpt_casp17_T.sh T2383 0 2
...
DEFAULT_R1_DIR="outputs/protenix/casp17_T_default_r1"
APPLIED_R1_DIR="outputs/protenix/casp17_T_applied_r1"
DEFAULT_R2_DIR="outputs/protenix/casp17_T_default_r2"
APPLIED_R2_DIR="outputs/protenix/casp17_T_applied_r2"

CANONICAL_R1="outputs/protenix/casp17_T_r1"
CANONICAL_R2="outputs/protenix/casp17_T_r2"

AUDIT_CSV="casp17/audit/protenix_ckpt_selection.csv"
LOG_DIR="logs/protenix/dualckpt"
mkdir -p "$LOG_DIR" "$(dirname $AUDIT_CSV)"

INPUT_JSON="data/test_cases/casp17_T/protenix_inputs/${TARGET}.json"

# ── Pre-flight ────────────────────────────────────────────────────────────
echo "=== Protenix dual-ckpt (T-series) for $TARGET (GPU $GPU_A=default, GPU $GPU_B=applied) ==="
...
PROTENIX_ROOT_DIR="$PROJECT_ROOT/weights/protenix" \
    conda run --no-capture-output -n casp17_ligand python \
    casp17_ligand/models/protenix_inference.py \
    --config-name protenix_inference_casp17_T_r2 \
